...
Claude 런타임에서 tool/MCP 노출까지 함께 확인하려면 아래 순서가 가장 안정적입니다.

```bash
# 1) Claude 세션에 실제로 노출된 tool 이름 목록 확인
OPENCODE_CONFIG_DIR="$HOME/.config/opencode-aegis" \
  opencode run --model anthropic/claude-sonnet-4-6 \
  "Return only the exact comma-separated names of all tools you can currently call."

# 2) 일반 tool 실제 호출 확인 (JSON 이벤트에 tool=read가 찍혀야 함)
OPENCODE_CONFIG_DIR="$HOME/.config/opencode-aegis" \
  opencode run --model anthropic/claude-sonnet-4-6 --format json \
  "Do not use bash. Use the read tool specifically on ./package.json and return only the package name."
```

권장 판정 기준:
- 1번 출력에 `context7_resolve-library-id`, `grep_app_searchGitHub`, `websearch_web_search_exa`, `memory_read_graph`, `sequential_thinking_sequentialthinking`가 보이면 MCP 노출은 정상
- 2번 JSON 이벤트에 `tool:"read"`가 보이면 실제 tool 호출도 정상

원격 MCP(`context7`, `grep_app`, `websearch`)와 일부 MCP 호출은 모델이 과도하게 명령형인 프롬프트를 prompt injection으로 오인하거나, 단순 질의에서는 굳이 tool call을 선택하지 않을 수 있습니다. 그래서 안정적인 smoke는 **tool list 노출 확인 + 안정적인 일반 tool(JSON `tool_use`) 확인** 기준으로 검증하는 것을 권장합니다. MCP 자체를 더 깊게 검증해야 한다면 실제 작업형 프롬프트를 주고 `--format json`에서 `tool_use` 이벤트를 확인하세요.
...
Claude 기본 모델은 `provider.anthropic` + `opencode-claude-auth`로 연결됩니다. `oh-my-aegis install`에서 Claude를 활성화했다면 아래 조건을 확인하세요.

- 로컬 `claude` CLI가 설치되어 있어야 합니다.
- `claude auth login` 등으로 Claude Code CLI 로그인 상태여야 합니다.
- 외부 `claude` CLI 호출 시 강제 안전 플래그: `--permission-mode=plan`, `--no-session-persistence`, `--tools=""`
- 위 플래그는 외부 Claude Code CLI 실행 경로에만 적용됩니다. Aegis 매니저 자체는 runtime policy에 따라 `skill/read/glob/grep/ast_grep_search/LSP/webfetch` 같은 safe discovery 도구와 `ctf_patch_audit`/`ctf_evidence_ledger` 같은 low-risk inspection·bounded-record 도구는 직접 사용할 수 있지만, `edit/bash`, `ctf_patch_apply`, `ctf_patch_review`, `ctf_patch_propose` 같은 실행·고권한 전이 계열 도구는 계속 차단됩니다.
...
| `anthropic/claude-sonnet-4-6` | Anthropic | `opencode-claude-auth` 또는 `ANTHROPIC_API_KEY` |
| `anthropic/claude-opus-4-6` | Anthropic | `opencode-claude-auth` 또는 `ANTHROPIC_API_KEY` |
| `anthropic/claude-haiku-4-5` | Anthropic | `opencode-claude-auth` 또는 `ANTHROPIC_API_KEY` |
| `anthropic/claude-sonnet-4.5` | Anthropic | `opencode-claude-auth` 또는 `ANTHROPIC_API_KEY` |
| `anthropic/claude-opus-4.1` | Anthropic | `opencode-claude-auth` 또는 `ANTHROPIC_API_KEY` |
