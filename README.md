...
- 위 플래그는 외부 Claude Code CLI 실행 경로에만 적용됩니다. Aegis 매니저 자체는 runtime policy에 따라 `skill/read/glob/grep/ast_grep_search/LSP` 같은 safe discovery 도구는 직접 사용할 수 있지만, `edit/bash/webfetch` 같은 실행·변경 계열 도구는 계속 차단됩니다.
