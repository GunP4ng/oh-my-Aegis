...
- **v0.2.16 (model_cli 기본값 확장 + lane role_profiles 시드)**: `model_cli` 기본 모델에 Gemini `gemini-3.1-pro`/`gemini-3-flash`(기존 `2.5-*` 유지)와 Claude `claude-sonnet-4.6`/`claude-opus-4.6`/`claude-haiku-4.5`를 포함하도록 확장했습니다. 또한 `oh-my-aegis install/apply`에서 `dynamic_model.role_profiles`를 시드하고 기존 사용자 설정과 deep-merge 하도록 정렬해 lane 기본 모델이 planning=4.6, exploration=3.1로 적용됩니다.
