...
import { scoreBenchmark, parseBenchmarkManifest } from "../src/benchmark/scoring";
...
function runDoctor(projectDir: string): DoctorReport {
  ...
  checks.push({
    ...
    message: existsSync(distIndexPath)
      ? `Found build artifact: ${distIndexPath}`
      : `Missing build artifact: ${distIndexPath}`,
  });
  ...
  checks.push({
    ...
    message: existsSync(fixturePath)
      ? `Found benchmark fixtures: ${fixturePath}`
      : `Missing benchmark fixtures: ${fixturePath}`,
  });
  ...
  if (!existsSync(resultsPath)) {
    ...
    try {
      ...
      checks.push({
        ...
        message:
          status === "pass"
            ? "Benchmark quality gate is perfect."
            : "Benchmark quality gate is needs_work. See missingDomains/missingEvidence.",
        details: {
